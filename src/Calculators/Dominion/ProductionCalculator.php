...
class ProductionCalculator
{
    ...
    public function getLumberProductionRaw(Dominion $dominion): float
    {
        ...
        // Unit Perk Production Bonus (Wood Elf Unit: Wisp)
        $lumber += $this->getUnitPerkProductionBonus($dominion, 'lumber_from_land');
    ...
    public function getOreProductionRaw(Dominion $dominion): float
    {
        ...
        $ore += $this->getUnitPerkProductionBonus($dominion, 'ore_production');
    ...
    /**
     * Returns the amount of resources generated by a Dominions's military units.
     *
     * @param Dominion $dominion
     * @param string $perk
     * @return float
     */
    public function getUnitPerkProductionBonus(Dominion $dominion, string $perk): float
    {
        $bonus = 0;

        foreach ($dominion->race->units as $unit) {
            $perkValue = $dominion->race->getUnitPerkValueForUnitSlot($unit->slot, $perk, null);
            ...
            if ($perkValue !== null) {
                ...
                $bonus += ($amountPerUnit * $dominion->{'military_unit' . $unit->slot});
            }
        }
