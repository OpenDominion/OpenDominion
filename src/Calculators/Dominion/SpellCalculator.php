...
class SpellCalculator
{
    ...
    public function getManaCost(Dominion $dominion, Spell $spell): int
    {
        ...
        // Mastery
        $maxMasteryBonus = -20;
        $spellCostMultiplier += min(1000, $dominion->wizard_mastery) / 1000 * $maxMasteryBonus / 100;
    ...
    public function getSpellCooldown(Dominion $dominion, Spell $spell): int
    {
        ...
        if ($spell->cooldown > 0) {
            $spellLastCast = DB::table('dominion_history')
                ...
                ->first();
            if ($spellLastCast) {
                $hoursSinceCast = (int) now()->startOfHour()->diffInHours(Carbon::parse($spellLastCast->created_at)->startOfHour());
