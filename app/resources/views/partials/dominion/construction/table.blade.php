        ...
        <tbody>
            @foreach ($buildingTypes as $buildingType)
                <tr>
                    ...
                    <td class="text-center">
                        <input type="number" name="construct[building_{{ $buildingType }}]" class="form-control text-center" placeholder="0" min="0" data-max="{{ $constructionCalculator->getMaxAfford($selectedDominion) }}" value="{{ old('construct.' . $buildingType) }}" {{ $selectedDominion->isLocked() ? 'disabled' : null }}>
                    </td>
                </tr>
            @endforeach
        </tbody>
